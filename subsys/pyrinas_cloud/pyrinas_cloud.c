...
{
  char data[] = {ota_cmd_type_check};
  uint16_t message_id = (uint16_t)sys_rand32_get();
  data_publish(ota_pub_topic, strlen(ota_pub_topic), data, sizeof(data), message_id);
}

static void publish_ota_done()
{
  char data[] = {ota_cmd_type_done};
  /* Save the message ID to check for an ACK */
  ota_done_message_id = (uint16_t)sys_rand32_get();

  printk("OTA done id:%d\n", ota_done_message_id);

  /* Publish the data */
  data_publish(ota_pub_topic, strlen(ota_pub_topic), data, sizeof(data), ota_done_message_id);
}
...
static void telemetry_check_event(struct k_timer *timer)
{
  ...
  {
    printk("Not connected. Will not start telemetry timer.\n");
  }
}
...
static void publish_evt_handler(const char *topic, size_t topic_len, const char *data, size_t data_len)
{
  ...
  if (strcmp(ota_sub_topic, topic) == 0)
  {
    ...
    if (((result == 0 && ota_data.force) || result == 1) && atomic_get(&ota_state_s) == ota_state_ready)
    {
      printk("Start upgrade\n");

      /* Start upgrade here*/
      k_delayed_work_submit(&fota_work, K_SECONDS(2));
    }
  }
}
